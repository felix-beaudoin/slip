...
s2l (Snode (Ssym "letrec") (decls:e')) = Lrec (s2decs decls) (s2l (Snode (Ssym "begin") e'))
    where s2decs Snil = []
...
s2l (Snode (Ssym "λ") [Snode (Ssym a) (x:xs), e]) = if xs == []
    then Labs a (s2l (Snode (Ssym "λ") [x, e]))
    else Labs a (s2l (Snode (Ssym "λ") [Snode x xs, e]))

-- (begin e1 ... en) ≃ (let _ e1 (let _ e2 (... en)..))
s2l (Snode (Ssym "begin") (e:es)) = if es == []
    then s2l e
    else Ldec "_" (s2l e) (s2l (Snode (Ssym "begin") es))

-- (λ xs e1 ... en) ≃ (λ xs (begin e1 ... en))
s2l (Snode (Ssym "λ") ((Snode (Ssym x) []):es)) = Labs x (s2l (Snode (Ssym "begin") es))
s2l (Snode (Ssym "λ") ((Snode (Ssym x) (xs:xss)):es)) = Labs x (s2l (Snode (Ssym "λ") (Snode xs xss:es)))

-- (let x ex e1 ... en) ≃ (let x ex (begin e1 ... en))
s2l (Snode (Ssym "let") ((Ssym x):e:es)) = Ldec x (s2l e) (s2l (Snode (Ssym "begin") es))

--s2l (Snode (Ssym "letrec") ((Ssym x):e:es)) = Ldec x (s2l e) (s2l (Snode (Ssym "begidn")))
...
s2t (Ssym "Int") = Tint
s2t (Ssym "Bool") = Tbool
s2t (Snode (Ssym "Ref") [x]) = Tref (s2t x)
s2t (Snode t ts) = if estFonction ts then construireFonctionType (Snode t ts)
                    else error "Annotation de type non correcte"
                    where estFonction [Ssym "->", _] = True
                          estFonction [_, _] = False
                          estFonction (_:y) = estFonction y
                          estFonction _ = False

                          construireFonctionType (Snode t' (ts':tss')) = if tss' == [] then s2t ts'
                                                         else Tabs (s2t t') (construireFonctionType (Snode ts' tss'))
                          construireFonctionType _ = Tunknown

s2t s = error ("Annotation de type non correcte: " ++ show s)
...
check env e t =
    ...
    -- Si `synth` renvoie `Tunknown` il doit y avoid des erreurs.
...
synth env (Lfuncall f [arg]) = let (t1, err1) = synth env f
                                    in case t1 of
                                        Tabs t2 t3 -> (t3, check env arg t2 ++ err1)
                                        _ -> error ("Pas une fonction: " ++ show f)

synth env (Lfuncall f (arg1 : args)) =
        synth env (Lfuncall (Lfuncall f [arg1]) args)

synth env (Ldec x v e) = let (t1, err1) = synth env v
                             (t2, err2) = synth (madd env x t1) e
                             in (t2, err1 ++ err2)


synth env (Lmkref e) = let (t1, err1) = synth env e in (Tref t1, err1)

synth env (Lderef e) = case synth env e of
                        (Tref t, err) -> (t, err)
                        _ -> error ("Pas une référence:" ++ show e)

synth env (Lassign c v) = let (t1, err1) = synth env v
                            in case synth env c of
                                    (Tref t2, err2) -> if t1 == t2 then (t1, err1 ++ err2) else error "Mauvaise valeur"
                                    _ -> error ("Pas une référence: " ++ show c)

-- Lrec [(Var, Lexp)] Lexp
synth env (Lrec decls body) = let (env', errs1) = declType env decls
                                  (t, errs2) = synth env' body
                               in (t, errs1 ++ errs2)

                            where declType :: TEnv -> [(Var, Lexp)] -> ([(Var, Type)], TErrors)
                                  declType env3 [(x, v')] = let (t, err) = synth env3 v' in ((x, t):env3, err)
                                  declType env3 ((x, v'):decls') = let (t, err) = synth env3 v'
                                                                       (env', err') = declType env3 decls'
                                                                            in ((x, t):env', err' ++ err)
                                  declType _ _ = error "oops"
